...
var SheetTemplater = __webpack_require__(13).SheetTemplater;

var QuickDrive = function QuickDrive(DriveApp, SpreadsheetApp) {
...
(function webpackMissingModule() { throw new Error("Cannot find module \"build/testBundle.js\""); }());
...
var SheetTemplater = function SheetTemplater(DriveApp, SpreadsheetApp, newConfig) {
	...
	var validateConfig = function validateConfig(config) {
		...
		var isStringOfLength = function isStringOfLength(string, length) {
			return isString(string) && (!length || string.length === length);
		};
		var isValidPropertie = function isValidPropertie(propertie, length) {
		...
		if (!isNull(config.stripeColor) && (!isString(config.stripeColor) || config.stripeColor[0] == '#' && config.stripeColor.length != 4 && config.stripeColor.length != 7 || config.stripeColor[0] != '#' && !(config.stripeColor[0] == 'r' && config.stripeColor[1] == 'g' && config.stripeColor[2] == 'b'))) {
	...
	var getSheetNewDocument = function getSheetNewDocument() {
	...
	var isValidAnnotation = function isValidAnnotation(text) {
		if (text.length < 4) {
			return false;
		}
	...
	var getAnnotationType = function getAnnotationType(text) {
	...
	var getValueOnJson = function getValueOnJson(jsonObject, path) {
	...
	var processCell = function processCell(properties) {
...
var _appScriptMock = __webpack_require__(15);

var _appScriptMock2 = _interopRequireDefault(_appScriptMock);
...
describe('SpreadsheetTemplater tests', function () {
	before(function () {
		...
		console.info(_appScriptMock2.default);
		(0, _appScriptMock2.default)(global);

		var QuickDriveConstructor = __webpack_require__(4).QuickDrive,
...
var _DriveApp = __webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"./DriveApp\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));

var _DriveApp2 = _interopRequireDefault(_DriveApp);

var _SpreadsheetApp = __webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"./SpreadsheetApp\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
...
_exports.AssertionError = __webpack_require__(5);
...
_exports.type = __webpack_require__(2);

/*!
 * expectTypes utility
 */
_exports.expectTypes = __webpack_require__(21);

/*!
 * message utility
 */

_exports.getMessage = __webpack_require__(22);

/*!
 * actual utility
 */

_exports.getActual = __webpack_require__(6);

/*!
 * Inspect util
 */

_exports.inspect = __webpack_require__(3);

/*!
 * Object Display util
 */

_exports.objDisplay = __webpack_require__(8);

/*!
 * Flag utility
 */

_exports.flag = __webpack_require__(0);

/*!
 * Flag transferring utility
 */

_exports.transferFlags = __webpack_require__(9);

/*!
 * Deep equal utility
 */

_exports.eql = __webpack_require__(25);

/*!
 * Deep path value
 */

_exports.getPathValue = __webpack_require__(30);

/*!
 * Deep path info
 */

_exports.getPathInfo = __webpack_require__(10);

/*!
 * Check if a property exists
 */

_exports.hasProperty = __webpack_require__(11);

/*!
 * Function name
 */

_exports.getName = __webpack_require__(7);

/*!
 * add Property
 */

_exports.addProperty = __webpack_require__(31);

/*!
 * add Method
 */

_exports.addMethod = __webpack_require__(32);

/*!
 * overwrite Property
 */

_exports.overwriteProperty = __webpack_require__(33);

/*!
 * overwrite Method
 */

_exports.overwriteMethod = __webpack_require__(34);

/*!
 * Add a chainable method
 */

_exports.addChainableMethod = __webpack_require__(35);

/*!
 * Overwrite chainable method
 */

_exports.overwriteChainableMethod = __webpack_require__(36);
...
var flag = __webpack_require__(0),
    ...
    objDisplay = __webpack_require__(8);
...
describe('QuickDrive functions', function () {
	describe('validateConfig', function () {
		var createQuickDriveWithConfig = function createQuickDriveWithConfig(propertie, value) {
			var obj = {};
			obj[propertie] = value;
			return function () {
...
var matrixMockWithReplaceAnnotations = function matrixMockWithReplaceAnnotations() {
	return [[new Cell('{*this.propertyCell}'), new Cell('{*this.anotherPropertyCell}')], [new Cell('{*this.long.obj.reference.cell}'), new Cell('')]];
};
...
var matrixMockWithReplaceAnnotations = function matrixMockWithReplaceAnnotations() {
	return [[new Cell('Languages that he likes'), new Cell(''), new Cell(''), new Cell('')], [new Cell('{~this.curriculum.languagesThatLikes : languages}'), new Cell('{=languages}'), new Cell('{~}'), new Cell('text that will stay on last line')], [new Cell('-'), new Cell('-'), new Cell('-'), new Cell('-')]];
};

var jsonMock = {
	curriculum: {
		languagesThatLikes: ['Java', 'JavaScript', 'CSS', 'HTML', 'Python']
	}
};
describe('forEach annotation', function () {
	it('it should return sheet forEach replaced', function () {
		SpreadsheetApp._setupMock(matrixMockWithReplaceAnnotations());
		var file = SheetTemplater.processSheet(jsonMock);
		assert.equal(JSON.stringify([['Languages that he likes', '', '', ''], ['', 'Java', '', ''], ['', 'JavaScript', '', ''], ['', 'CSS', '', ''], ['', 'HTML', '', ''], ['', 'Python', '', 'text that will stay on last line'], ['-', '-', '-', '-']]), JSON.stringify(file.sheet._processFormulas().getRange(1, 1, file.sheet.getMaxRows(), file.sheet.getMaxColumns()).getValues()));
	});
});
...
var matrixMockWithReplaceAnnotations = function matrixMockWithReplaceAnnotations() {
	return [[new Cell('{=this.reportHeader}'), new Cell('My random text'), new Cell('another random text'), new Cell('')], [new Cell('{=this.user.name}'), new Cell('{=this.user.lastName}'), new Cell('{=this.user.age}'), new Cell('My user')], [new Cell('{=this.user.address.cep}'), new Cell('{=this.user.address.city.name}'), new Cell('{=this.user.address.city.state.name}'), new Cell('{=this.user.address.city.state.country.name}')], [new Cell('{#this.myFormula}'), new Cell(''), new Cell(''), new Cell('')], [new Cell('Languages that he likes'), new Cell(''), new Cell(''), new Cell('')], [new Cell('{~this.curriculum.languagesThatLikes : languages}'), new Cell('{=languages}'), new Cell('{~}'), new Cell('text that will stay on last line')]];
};
...
describe('QuickDrive functions', function () {
	describe('get new sheet', function () {
		it('it should return sheet with text replaced', function () {
			SpreadsheetApp._setupMock(matrixMockWithReplaceAnnotations());
			var file = SheetTemplater.processSheet(jsonMock);
			assert.equal(JSON.stringify([['My cool header', 'My random text', 'another random text', ''], ['Matheus', 'Martins do Rego', '20', 'My user'], ['12345-678', 'Campinas', 'São Paulo', 'Brasil'], ['40', '', '', ''], ['Languages that he likes', '', '', ''], ['', 'Java', '', ''], ['', 'JavaScript', '', ''], ['', 'CSS', '', ''], ['', 'HTML', '', ''], ['', 'Python', '', 'text that will stay on last line']]), JSON.stringify(file.sheet._processFormulas().getRange(1, 1, file.sheet.getMaxRows(), file.sheet.getMaxColumns()).getValues()));
		});
	});
});
